...
int	find_rot_cnt_from_top(t_admin *master, char stack_name, int l)
{
	...
	stack = plug_top_ptr(master, stack_name);
	if (l == 0)
		while (stack->num >= master->partitions[0])
			stack = stack->next;
	else if (l == master->div_num - 1)
	{
		while (stack->num < master->partitions[master->div_num - 2])
			stack = stack->next;
	}
	else
	{
		while ((stack->num < master->partitions[l - 1])
			|| (stack->num >= master->partitions[l]))
			stack = stack->next;
	}
	return (find_rot_cnt_to_top(master, stack, stack_name));
}
